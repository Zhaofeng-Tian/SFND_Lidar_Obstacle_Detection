...
struct KdTree
{
	...
	/************************************************/
	void searchNeighbor(std::vector<int> &ids, pcl::PointXYZI target,
		...
		if (parent_node != NULL) {
			...
			if (depth % 3 == 0) // 3 dim kd-tree - x-axis
			{
				if (-distanceTol < delta_x)
					searchNeighbor(ids, target, distanceTol, parent_node->left, depth+1);
				if (distanceTol > delta_x)
					searchNeighbor(ids, target, distanceTol, parent_node->right, depth+1);
			}
	...
	std::vector<int> search(pcl::PointXYZI target, float distanceTol) {
		std::vector<int> ids;
		searchNeighbor(ids, target, distanceTol, root, 0);
		return ids;
	}
